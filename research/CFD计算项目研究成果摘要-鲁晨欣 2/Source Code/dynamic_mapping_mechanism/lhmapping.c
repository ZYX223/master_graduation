...
static thread_group_t groups[3][MAX_THREADS]; 
...
void generate_group(comm_matrix_t *m, uint32_t total_elements, uint32_t group_elements, thread_group_t *group, uint32_t level, uint32_t *chosen)
{
	...
	for (i=0; i<group_elements; i++) { // 分组内每一个元素，循环寻找最适合的元素
		wmax = -1;
		for (j=0; j<total_elements; j++) { // 循环计算每一个待分组元素的通信量，找到最大的元素
			if (!chosen[j]) {
				w = 0;
				for (k=0; k<i; k++) {
...
uint32_t generate_groups(comm_matrix_t *m, uint32_t nelements, uint32_t level)
{
	...
	for (done=0; done<nelements; done+=in_group) { //循环分每一个组
...
void map_groups_to_topology_ (vertex_t *v, vertex_t *from, thread_group_t *g, uint32_t *map, uint32_t level)
{
	...
	if (v->type == GRAPH_ELTYPE_PU) {
	...
	else if ((v->type == GRAPH_ELTYPE_ROOT && v->arity >= 2) || v->arity > 2) { // 硬件对象是可共享的
		for (i=0; i<g->nelements; i++) {
		...
		if (v->type == GRAPH_ELTYPE_ROOT) { // 非共享根节点
...
//初始化输入
void* lhmapping_mapping_algorithm_greedy_init (thread_map_alg_init_t *data)
{
	uint32_t pos, i;

	hardware_topology = data->topology;
	levels_n = 2;
	lhmapping_topology_walk_pre_order(data->topology, detect_levels_with_sharers, NULL);

	arities[0] = arities[1] = 1; // first arities should always be 1
	pos = levels_n - 1;
	lhmapping_topology_walk_pre_order(data->topology, detect_arity_of_levels_with_sharers, &pos);
...
//顶层算法，结果保存至map
void lhmapping_mapping_algorithm_greedy_map (thread_map_alg_map_t *data)
{
