...
//检测page fault, 遍历页表，引入extra page fault
struct pf_process_t {
...
int dmmo_pagefault_func(void* v)
{
	...
	while (1) {
		...
		if (dmmo_get_active_threads() < 4) {//队列大小
			continue;
		}
		for (i=0; i<dmmo_get_active_threads(); i++) {
			struct task_struct *tsk = pid_task(find_vpid(dmmo_get_pid(i)), PIDTYPE_PID);
			if (tsk)
...
struct vm_area_struct* find_next_vma(long pid, struct mm_struct *mm, struct vm_area_struct* prev_vma)
{
	...
	while (1) {
		...
		if (tmp && (!dmmo_vma_shared_flag || is_shared(tmp)))
			return tmp;
	}
}
...
void dmmo_pf_thread_init(void)
{
	...
	if (!proc)
		printk("DMMO BUG: could not allocate proc array\n");
	else
	...
	if (!walk_page_range_p) {
		...
		if (!walk_page_range_p)
			printk("DMMO BUG: walk_page_range missing\n");
	}
}
