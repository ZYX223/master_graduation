...
unsigned long dmmo_pf;
unsigned long dmmo_pte_fixes;
int dmmo_vma_shared_flag = 1;
							...
							pte_t *pte, pmd_t *pmd, unsigned int flags)
{
	...
	if (tid != -1){
		...
		if (physaddr)
			dmmo_check_comm(tid, (physaddr<<PAGE_SHIFT) | (address & (PAGE_SIZE-1)));
	}
...
{
	int at, tid = dmmo_get_tid(tsk->pid);

	if (tid > -1 && (tsk->flags & PF_EXITING)) {
		dmmo_delete_pid(tsk->pid);
		at = dmmo_get_active_threads();
		printk("DMMO: %s stop (pid %d, tid %d), #active: %d\n", tsk->comm, tsk->pid, tid, at);
		if (at == 0) {
			printk("DMMO: stop app %s (pid %d, tid %d)\n", tsk->comm, tsk->pid, tid);
			print_stats();
			reset_stats();
		}
		jprobe_return();
	...
	if (check_name(tsk->comm) && tid == -1 && !(tsk->flags & PF_EXITING)) {
		if (dmmo_get_active_threads() == 0) {
			dmmo_pid_clear();
			dmmo_comm_init();
		}
		tid = dmmo_add_pid(tsk->pid);
		printk("DMMO: new process %s (pid %d, tid %d); #active: %d\n", tsk->comm, tsk->pid, tid, dmmo_get_active_threads());
	}
...
int thread_handler(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	if (check_name(current->comm) && pid > 0) {
		int tid = dmmo_add_pid(pid);
		dmmo_vma_shared_flag = 0;
		printk("DMMO: new thread %s (pid:%d, tid:%d); #active: %d\n", current->comm, pid, tid, dmmo_get_active_threads());
	}
...
void dmmo_probes_init(void)
{
	...
	if ((ret=register_jprobe(&pte_fault_probe))) {
		printk("DMMO BUG: handle_pte_fault missing, %d\n", ret);
	}
	if ((ret=register_jprobe(&process_probe))){
		printk("DMMO BUG: acct_update_integrals missing, %d\n", ret);
	}
	if ((ret=register_kretprobe(&thread_probe))){
		printk("DMMO BUG: do_fork missing, %d\n", ret);
	}
	...
	if (do_pf) {
		...
		if ((ret=register_jprobe(&del_page_probe))){
			printk("DMMO BUG: __delete_from_page_cache missing, %d\n", ret);
		}
		if ((ret=register_jprobe(&unmap_page_range_probe))){
			printk("DMMO BUG: unmap_page_range missing, %d\n", ret);
		}
	}
}
